...
public class Penjat {
        ...
        if (numero != 0) {
            for (int i = 0; i < paraulaEnJoc.length(); i++) {
                if (paraulaEnJoc.charAt(i) == letra) {
                    ...
                    oculto = oculto + "*";
                        throw new CustomException("Swiper no copies, el gat Renat no te lo perdonaria nunca");
                }
            }
        }
    ...
    public static boolean prou(int jugadas, int ganadas, int perdidas, int canceladas) {
        ...
        if (finalitza.equals("si")) {
            System.out.println("Paraules jugades: " + jugades);
            System.out.println("Paraules encertades: " + ganades);
            System.out.println("Paraules fallades: " + perdides);
...
//El Moisés sabe que te estas copiando bobo, ponte a trabajar vago
